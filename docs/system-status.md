...
Local Jev combat bot: `/tools/jev.html` now runs in real time only, without a request budget. Jev selects targets/tactics; a bounded normal-input motor handles body-safe routes, melee/ranged spacing, repeated basics and imminent-threat evasion. Sword, bow and wand fixtures offer five compatible skills, target memory, search and progress feedback. North-archer and wall encounters reproduce pursuit/routing cases. Twenty-eight focused tests and nine frozen live-model probes cover the new behavior; sustained gameplay remains user-tested. The shared controller now closes inside melee reach while swinging, routes to reachable attack positions beside wall-hugging enemies, and reaches wall-corner waypoints without stopping short. No playable saves or production entrypoints are connected. See [Jev bot behavior and evidence](jev-arena.md).
...
| Keyboard / mouse controls | Dedicated controls screen; saved primary/alternate bindings; four outlined gameplay cursors, saved 50–250% size and compact actual-size preview; explicit conflict replacement, unbind/reset; shared live HUD/atlas/interaction labels | Local device preferences; chosen cursor size stays fixed on screen above CRT; controller layout remains fixed; see [controls](controls.md) |
...
| Equipment | Twelve kinds, six rarities, eleven slots; 17 generated weapons, 3 shields and 6 caster foci; shared affixes with curated caster rolls; visible procedural gear | Recipe-based +10 enhancement and enchanting; twelve fixed Unique designs with skill powers |
...
| Skill atlas | Local 1,824-node, 2,118-edge open network; 144 passive neighborhoods, 30 skills, 90 Techniques, eight Doctrine families, four keystones and twenty ranks; stronger passive rewards and validated previous-tree refunds | Published v0.3.17 remains the 875-node atlas; open network is local and awaits further player balance feedback |
